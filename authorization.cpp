...
int Authorized::authorized(int work_sock, std::string file_name, std::string file_error) {
    try {
        ...
        file.open(file_name);
        if (!file.is_open()) {
            throw RuntimeError("Failed to open file: " + file_name);
        }

        getline(file, login, ':');
        getline(file, hashq);
        std::cout << "Read login from file: " << login << std::endl;
        int len_log = login.length();
        ...
        std::string client_login = message.substr(0, len_log);
        std::string client_salt = message.substr(len_log, 16);
        std::string client_hash = message.substr(len_log + 16);
        ...
        if (client_login != login) {
            ...
            this->e_error.errors(error, file_error);
            close(work_sock);
            return 1;
        } else {
